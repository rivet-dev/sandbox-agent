...
async function validateReuseVersion(version: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`${version} does not exist in git. Make sure ${ref} exists in the repository.`);
  }
  ...
  if (!Array.isArray(files?.Contents) || files.Contents.length === 0) {
    throw new Error(`No S3 artifacts found for version ${version} (commit ${shortCommit}) under ${commitPrefix}`);
  }
...
async function getVersionFromArgs(opts: { version?: string; major?: boolean; minor?: boolean; patch?: boolean }): Promise<string> {
  ...
  if (!opts.major && !opts.minor && !opts.patch) {
    throw new Error("Must provide either --version, --major, --minor, or --patch");
  }

  // Get latest version from git tags and calculate new one
  const latestVersion = await getLatestGitVersion();
  if (!latestVersion) {
    throw new Error("No existing version tags found. Use --version to set an explicit version.");
  }
...
const PHASE_MAP: Record<Phase, Step[]> = {
  ...
  "setup-local": ["confirm-release", "update-version", "run-local-checks", "git-commit", "git-push", "trigger-workflow"],
...
async function main() {
  ...
  program
    ...
    .option("--override-commit <commit>", "Override the commit to pull artifacts from (defaults to current commit)")
    .option(
    ...
    .option("--only-steps <steps>", `Run specific steps (comma-separated). Available: ${STEPS.join(", ")}`)
    .option("--phase <phase>", `Run a release phase (comma-separated). Available: ${PHASES.join(", ")}`)
    .parse();
  ...
  if (!opts.phase && !opts.onlySteps) {
    throw new Error("Must provide either --phase or --only-steps. Run with --help for more information.");
  }
  ...
  if (opts.onlySteps) {
    ...
    for (const step of steps) {
      if (!STEPS.includes(step as Step)) {
        throw new Error(`Invalid step: ${step}. Available steps: ${STEPS.join(", ")}`);
      }
    ...
    for (const phase of phases) {
      if (!PHASES.includes(phase as Phase)) {
        throw new Error(`Invalid phase: ${phase}. Available phases: ${PHASES.join(", ")}`);
      }
