...
impl AcpDispatch for AcpProxyRuntime {
    fn post(
        ...
        Box::pin(async move {
            match self.post(&server_id, agent, payload).await {
                Ok(ProxyPostOutcome::Response(value)) => {
                    Ok(AcpDispatchResult::Response(value))
                }
    ...
    ) -> Pin<Box<dyn Future<Output = Result<AcpPayloadStream, String>> + Send + '_>>
    {
        let server_id = server_id.to_string();
        Box::pin(async move {
            let instance = self.get_instance(&server_id).await.map_err(|e| e.to_string())?;
    ...
    fn delete(
        ...
        let server_id = server_id.to_string();
        Box::pin(async move {
            self.delete(&server_id).await.map_err(|err| err.to_string())
        })
    }
}
