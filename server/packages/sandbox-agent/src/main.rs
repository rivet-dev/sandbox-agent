...
use sandbox_agent::router::{
    AgentInstallRequest, AgentListResponse, AgentModelsResponse, AgentModesResponse, AppState,
    AuthConfig, CreateSessionRequest, CreateSessionResponse, EventsResponse, FsActionResponse,
    FsEntry, FsMoveRequest, FsMoveResponse, FsStat, FsUploadBatchResponse, FsWriteResponse,
    McpServerConfig, MessageRequest, PermissionReply, PermissionReplyRequest, QuestionReplyRequest,
    SessionListResponse, SkillsConfig,
};
...
enum ApiCommand {
    ...
    /// Manage files in the sandbox filesystem.
    Fs(FsArgs),
}
...
fn run_sessions(command: &SessionsCommand, cli: &Cli) -> Result<(), CliError> {
    match command {
        ...
        SessionsCommand::Create(args) => {
            let ctx = ClientContext::new(cli, &args.client)?;
            let mcp = if let Some(path) = args.mcp_config.as_ref() {
                let text = std::fs::read_to_string(path)?;
                let parsed: BTreeMap<String, McpServerConfig> = serde_json::from_str(&text)?;
                Some(parsed)
            } else {
                None
            };
...
fn run_fs(command: &FsCommand, cli: &Cli) -> Result<(), CliError> {
    match command {
            ...
            let body = match (&args.content, &args.from_file) {
                (Some(_), Some(_)) => {
                    return Err(CliError::Server(
                        "use either --content or --from-file, not both".to_string(),
                    ))
                }
                (None, None) => {
                    return Err(CliError::Server(
                        "missing content: provide --content or --from-file".to_string(),
                    ))
                }
        ...
        FsCommand::UploadBatch(args) => {
            let ctx = ClientContext::new(cli, &args.client)?;
            let bytes = std::fs::read(&args.tar)?;
...
impl ClientContext {
    ...
    fn post_raw_with_query(
        ...
        let mut request = self.request(Method::POST, path).header("content-type", content_type);
        for (key, value) in query {
            if let Some(value) = value {
                request = request.query(&[(key, value)]);
            }
