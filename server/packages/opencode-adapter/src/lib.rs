...
async fn oc_session_delete(
    ...
    if state.acp_initialized.lock().await.remove(&server_id).is_some() {
        if let Some(dispatch) = state.config.acp_dispatch.as_ref() {
            if let Err(err) = dispatch.delete(&server_id).await {
                warn!(?err, "failed to delete ACP server instance on session delete");
            }
        }
    }
...
async fn oc_session_abort(
    ...
    if let Some(dispatch) = state.config.acp_dispatch.as_ref() {
        ...
        if let Some(server_id) = agent_session_id {
            let acp_session_id = state.acp_initialized.lock().await
                .get(&server_id)
                .cloned();
        ...
        if meta.agent != "mock" {
            ...
            if needs_init {
                ...
                match dispatch.notification_stream(&server_id, None).await {
                    ...
                    Err(err) => {
                        warn!(?err, "failed to open ACP SSE stream; events will not be translated");
                    }
                }

                state.acp_initialized.lock().await.insert(server_id.clone(), acp_session_id);
            }
...
async fn translate_session_update(
        ...
        )
    {
        ...
        let info = build_assistant_message(
            session_id, message_id, &parent_id, now, directory, agent, provider_id, model_id,
        );
        state.emit_event(message_event("message.updated", &info));
