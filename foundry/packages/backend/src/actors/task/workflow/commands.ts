...
export async function handleSwitchActivity(loopCtx: any, msg: any): Promise<void> {
  const db = loopCtx.db;
  const runtime = await db.select({ switchTarget: taskRuntime.activeSwitchTarget }).from(taskRuntime).where(eq(taskRuntime.id, HANDOFF_ROW_ID)).get();
...
export async function handleSimpleCommandActivity(loopCtx: any, msg: any, statusMessage: string, historyKind: string): Promise<void> {
  const db = loopCtx.db;
  await db.update(taskRuntime).set({ statusMessage, updatedAt: Date.now() }).where(eq(taskRuntime.id, HANDOFF_ROW_ID)).run();
...
export async function handleArchiveActivity(loopCtx: any, msg: any): Promise<void> {
  ...
  await db.update(taskTable).set({ status: "archived", updatedAt: Date.now() }).where(eq(taskTable.id, HANDOFF_ROW_ID)).run();

  await db.update(taskRuntime).set({ activeSessionId: null, statusMessage: "archived", updatedAt: Date.now() }).where(eq(taskRuntime.id, HANDOFF_ROW_ID)).run();
...
export async function killWriteDbActivity(loopCtx: any, msg: any): Promise<void> {
  ...
  const db = loopCtx.db;
  await db.update(taskTable).set({ status: "killed", updatedAt: Date.now() }).where(eq(taskTable.id, HANDOFF_ROW_ID)).run();

  await db.update(taskRuntime).set({ statusMessage: "killed", updatedAt: Date.now() }).where(eq(taskRuntime.id, HANDOFF_ROW_ID)).run();
