...
export async function statusUpdateActivity(loopCtx: any, body: any): Promise<boolean> {
  ...
  if (isActive) {
    await db.update(taskTable).set({ status: newStatus, updatedAt: body.at }).where(eq(taskTable.id, HANDOFF_ROW_ID)).run();
...
export async function idleSubmitPrActivity(loopCtx: any): Promise<void> {
  ...
  const self = await db.select({ prSubmitted: taskTable.prSubmitted }).from(taskTable).where(eq(taskTable.id, HANDOFF_ROW_ID)).get();
  ...
  try {
    ...
    await db.update(taskTable).set({ prSubmitted: 1, updatedAt: Date.now() }).where(eq(taskTable.id, HANDOFF_ROW_ID)).run();
