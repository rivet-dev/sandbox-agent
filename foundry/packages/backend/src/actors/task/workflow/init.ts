...
import { getOrCreateTaskStatusSync, getOrCreateHistory, getOrCreateRepo, getOrCreateSandboxInstance, selfTask } from "../../handles.js";
import { logActorWarning, resolveErrorMessage } from "../../logging.js";
import { task as taskTable, taskRuntime, taskSandboxes } from "../db/schema.js";
import { HANDOFF_ROW_ID, appendHistory, collectErrorMessages, resolveErrorDetail, setTaskState } from "./common.js";
...
export async function initEnsureNameActivity(loopCtx: any): Promise<void> {
  ...
  const repo = await getOrCreateRepo(loopCtx, loopCtx.state.workspaceId, loopCtx.state.repoId, loopCtx.state.repoRemote);
...
export async function initCreateSandboxActivity(loopCtx: any, body: any): Promise<any> {
  ...
  if (provider.capabilities().supportsSessionReuse) {
    const runtime = await loopCtx.db.select({ activeSandboxId: taskRuntime.activeSandboxId }).from(taskRuntime).where(eq(taskRuntime.id, HANDOFF_ROW_ID)).get();
...
export async function initWriteDbActivity(
  ...
  const statusMessage = sessionDeferred
    ? "ready"
    : sessionHealthy
      ? "session created"
