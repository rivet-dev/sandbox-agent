...
const scriptFile = path.resolve(__dirname, "../dist/list-files.cjs");
if (!fs.existsSync(scriptFile)) {
  console.error("Error: dist/list-files.cjs not found. Run `pnpm build:script` first.");
  process.exit(1);
}
console.log("Step 1: Script bundle ready at dist/list-files.cjs");

// Step 2: Start Docker container with sandbox-agent + Node.js
console.log("Step 2: Starting Docker container...");
const { baseUrl, cleanup } = await startDockerSandbox({ port: 3005, packages: ["nodejs"] });

// Step 3: Upload bundled script and SKILL.md to sandbox
console.log("Step 3: Uploading script and skill file...");
...
console.log("Step 4: Creating session with custom skill...");
console.log('  Try: "list the files in /opt"');
